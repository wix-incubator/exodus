...
class MavenBuildSystem(repoRoot: Path,
  ...
  private val dependencyResolver: MavenDependencyResolver = maybeDependencyResolver.getOrElse(new AetherMavenDependencyResolver(remoteMavenRepositoryUrls, ignoreMissingDependenciesFlag = true))
