...
class ImportExternalTargetsFileTest extends SpecificationWithJUnit {
  ...
  "Import External Targets File builder" should {
    ...
    "leave empty file if header is funky" in {
      val fileWithANormalHeader = s"""load("//:import_external.bzl", import_external = "safe_wix_scala_maven_import_external")
                         |
                         |def dependencies():
                         |
      ...
      val fileWithAnotherFunkyHeader = s"""load("//:import_external.bzl", import_external = "wix_snapshot_scala_maven_import_external")
                                    |def dependencies():
                                    |
                                    |${importExternalRuleWith(artifactA).serialized}""".stripMargin
      ...
      val fileWithYetAnotherFunkyHeader = s"""load("//:import_external.bzl", import_external = "wix_snapshot_scala_maven_import_external")
                                        |load("//:import_external.bzl", import_external_no_src = "safe_wix_scala_maven_import_external")
                                        |
                                        |def dependencies():
                                        |
                                        |${importExternalRuleWith(artifactA).serialized}""".stripMargin
