...
class BazelDependenciesWriterTest extends SpecificationWithJUnit {
  ...
  "BazelDependenciesWriter " >> {
    ...
    trait emptyThirdPartyReposCtx extends Scope {
      ...
      def writer = writerFor(
        localWorkspace,
        testOnlyTargetsResolver = TestOnlyTargetsResolverForTest)
    ...
    "given one new dependency with transitive dependencies" should {
      ...
      "write target with runtime dependencies from overrides" in new dependencyWithTransitiveDependencyofScope(MavenScope.Runtime) {
        ...
        override def writer: BazelDependenciesWriter = writerFor(
          localWorkspace,
          testOnlyTargetsResolver = TestOnlyTargetsResolverForTest
        )
  ...
  private def writerFor(localWorkspace: BazelLocalWorkspace,
                        ...
                        testOnlyTargetsResolver: TestOnlyTargetsResolver) = {
    ...
    new BazelDependenciesWriter(localWorkspace, maybeManagedDepsRepoPath = None, neverLinkResolver, statement)
  }
...
object TestOnlyTargetsResolverForTest extends TestOnlyTargetsResolver {
  override def isTestOnlyTarget(artifactCoordinates: Coordinates): Boolean = false
}
